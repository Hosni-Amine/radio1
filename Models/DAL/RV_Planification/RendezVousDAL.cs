...
namespace radio1.Models.DAL.RV_Planification
{
	public class RendezVousDAL
	{
		...
		public static List<RendezVous> GetAll(Users user)
		{
			...
			{
				string sqlstr = "SELECT rv.* , type_op.Nom AS TypeOperationName ,type_op.Id AS TypeOperation_Id, p.Nom AS PatientName ,p.Prenom AS PatientName2 , p.Telephone AS PatientPhone ,  d.Nom AS DoctorName ,d.Prenom AS DoctorName2 , d.Email AS DoctorEmail , t.Nom AS TechnicienName , t.Prenom AS TechnicienName2 , t.Email AS TechnicienEmail , s.Nom AS SecretaireName , s.Prenom AS SecretaireName2  ,s.User_Id AS Sec_User_Id , s.Email AS SecretaireEmail, a.numserie AS AppareilRadioNumSerie FROM RendezVous rv LEFT JOIN Doctor d ON rv.DoctorId = d.Id LEFT JOIN Technicien t ON rv.TechnicienId = t.Id LEFT JOIN Patient p ON rv.PatientId = p.Id LEFT JOIN TypeOperation type_op ON rv.TypeOperationId = type_op.Id LEFT JOIN Secretaire s ON rv.SecretaireId = s.Id LEFT JOIN AppareilRadio a ON type_op.AppareilRadioId = a.Id WHERE rv.PatientId IN (SELECT Id FROM Patient WHERE Id = @UserId) ORDER BY rv.date";
				command = new SqlCommand(sqlstr, connection);
				command.Parameters.AddWithValue("@UserId", user.Id);
			}
			else
			{
				string sqlstr = "SELECT rv.* , type_op.Nom AS TypeOperationName ,type_op.Id AS TypeOperation_Id, p.Nom AS PatientName ,p.Prenom AS PatientName2 , p.Telephone AS PatientPhone ,  d.Nom AS DoctorName ,d.Prenom AS DoctorName2 , d.Email AS DoctorEmail , t.Nom AS TechnicienName , t.Prenom AS TechnicienName2 , t.Email AS TechnicienEmail , s.Nom AS SecretaireName , s.Prenom AS SecretaireName2  , s.Email AS SecretaireEmail, a.numserie AS AppareilRadioNumSerie FROM RendezVous rv LEFT JOIN Doctor d ON rv.DoctorId = d.Id LEFT JOIN Technicien t ON rv.TechnicienId = t.Id LEFT JOIN Patient p ON rv.PatientId = p.Id LEFT JOIN TypeOperation type_op ON rv.TypeOperationId = type_op.Id LEFT JOIN Secretaire s ON rv.SecretaireId = s.Id LEFT JOIN AppareilRadio a ON type_op.AppareilRadioId = a.Id ORDER BY rv.date;";
				command = new SqlCommand(sqlstr, connection);
			}
			DataTable table = new DataTable();
			SqlDataReader reader = command.ExecuteReader();
		...
		public static RendezVous Get(DataRow raw)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(raw["Sec_User_Id"].ToString()))
				{
					rendezvous.secretaire.User_Id = Convert.ToString(raw["Sec_User_Id"]);
				}
