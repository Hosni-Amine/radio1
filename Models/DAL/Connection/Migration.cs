...
namespace radio1.Models.DAL.Connection
{
    public class Migration
    {
        ...
        public static void CreateRendezVousTableIfNotExists()
        {
            ...
            SqlCommand command = new SqlCommand(sqlstrTypeR, connection);
            ...
            command = new SqlCommand(sqlstrTypeR, connection);
            command.ExecuteNonQuery();
            ...
            command = new SqlCommand(sqlstrTypeR, connection);
            command.ExecuteNonQuery(); 
            sqlstrTypeR = "IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'RendezVous') BEGIN CREATE TABLE RendezVous (Id INT IDENTITY(1,1) PRIMARY KEY, Date DATETIME, Status NVARCHAR(10) , Examen NVARCHAR(50), Image_Name NVARCHAR(50) NULL,Inter_PDF NVARCHAR(50) NULL, Inter_Vocal NVARCHAR(50) NULL , Interpretation NVARCHAR(MAX) NULL , PatientId INT NULL , TypeOperationId INT NULL, DoctorId INT NULL, TechnicienId INT NULL, SecretaireId INT NULL, FOREIGN KEY(TypeOperationId) REFERENCES TypeOperation(Id) ON DELETE SET NULL, FOREIGN KEY(DoctorId) REFERENCES Doctor(Id) ON DELETE SET NULL, FOREIGN KEY(PatientId) REFERENCES Patient(Id) ON DELETE SET NULL , FOREIGN KEY(TechnicienId) REFERENCES Technicien(Id) ON DELETE SET NULL, FOREIGN KEY(SecretaireId) REFERENCES Secretaire(Id) ON DELETE SET NULL, CONSTRAINT uc_RendezVous UNIQUE(Date, PatientId, Image_Name)) END";
            command = new SqlCommand(sqlstrTypeR, connection);
            command.ExecuteNonQuery();
            connection.Close();
